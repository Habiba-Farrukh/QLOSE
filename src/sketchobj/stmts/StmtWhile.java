...
public class StmtWhile extends Statement {
	...
	public StmtWhile clone() {
		return new StmtWhile(this.cond.clone(), this.body.clone(), this.getLineNumber());
	}
	...
	@Override
	public Context buildContext(Context prectx, int sp, List<Statement> originalStatements) {
		...
		postctx = body.buildContext(postctx, sp+1, originalStatements);
		postctx.popVars();
	...
	public Map<String, Type> addRecordStmt(StmtBlock parent, int index, Map<String, Type> m) {
		...
		body = new StmtBlock(body,ConstraintFactory.recordState(body.getPostctx().getLinenumber(),
				body.getPostctx().getAllVars()));
	...
	public void addRecordStmtOriginal(StmtBlock parent, int index) {
		...
		body = new StmtBlock(body,ConstraintFactory.recordStateOriginal(body.getPostctx().getLinenumber(),
				body.getPostctx().getAllVars()));
		((StmtBlock)body).stmts.get(0).addRecordStmtOriginal((StmtBlock) body, 0);
	}
