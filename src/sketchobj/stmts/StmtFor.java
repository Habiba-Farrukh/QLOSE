...
public class StmtFor extends Statement {
	...
	@Override
	public Context buildContext(Context prectx, int position, List<Statement> originalStatement) {
		...
		postctx = init.buildContext(postctx, position, originalStatement);
		cond.setCtx(postctx);
		postctx = incr.buildContext(postctx, position, originalStatement);
		postctx = body.buildContext(postctx,position +1, originalStatement);
		postctx.popVars();
		postctx.setVarsInScope(new ArrayList<String>());
		return postctx;
	...
	public Map<String, Type> addRecordStmt(StmtBlock parent, int index, Map<String, Type> m) {
		...
		StmtBlock sb = new StmtBlock(ConstraintFactory.recordState(this.getPrectx().getLinenumber(),
				this.getPrectx().getAllVars()),this);
		sb = new StmtBlock(sb, ConstraintFactory.recordState(this.getPostctx().getLinenumber(),
				this.getPostctx().getAllVars()));
		...
		return ((StmtBlock)((StmtBlock) body).stmts.get(0)).stmts.get(((StmtBlock)((StmtBlock) body).
				stmts.get(0)).stmts.size()-1).addRecordStmt((StmtBlock) body, 1, m);
	}
	...
	public void addRecordStmtOriginal(StmtBlock parent, int index) {
		StmtBlock sb 	  = new StmtBlock(ConstraintFactory.recordStateOriginal(this.getPrectx().getLinenumber(),
							this.getPrectx().getAllVars()),this);
		sb 				  = new StmtBlock(sb, ConstraintFactory.recordStateOriginal(this.getPostctx().getLinenumber(),
							this.getPostctx().getAllVars()));
		parent.stmts.set(index, sb);
		body 			  = new StmtBlock(ConstraintFactory.recordStateOriginal(this.getPostctx().getLinenumber(),
							init.getPostctx().getAllVars()), body);
		body 		  	  =  new StmtBlock(body,ConstraintFactory.recordStateOriginal(this.getPostctx().getLinenumber(),
							init.getPostctx().getAllVars()));
		((StmtBlock)((StmtBlock) body).stmts.get(0)).stmts.get(((StmtBlock)((StmtBlock) body).
				stmts.get(0)).stmts.size()-1).addRecordStmtOriginal((StmtBlock) body, 1);
	}
